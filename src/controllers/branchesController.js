...
const branchesController = {
  createBranch: async (req, res) => {
    ...
    if (!validate.phone(phoneNumber)) {
      return res
        .status(400)
        .json({ message: 'Formato de número de teléfono inválido.' });
    }
    if (!address) {
      return res
        .status(400)
        .json({ message: 'La dirección postal es obligatoria.' });
    }
    ...
    if (!capacity) {
      return res
        .status(400)
        .json({ message: 'La capacidad de la sucursal es obligatoria.' });
    }
    ...
    if (!openingTime) {
      return res
        .status(400)
        .json({ message: 'La hora de apertura es obligatoria.' });
    }
    ...
    if (!closingTime) {
      return res
        .status(400)
        .json({ message: 'La hora de cierre es obligatoria.' });
    }
    ...
    if (!turnDuration) {
      return res
        .status(400)
        .json({ message: 'Se requiere completar la duración de turno.' });
    }
    if (!validate.turnDuration(turnDuration)) {
      return res.status(400).json({
        message:
          'Duración de turno inválida. Tiene que ser mayor a 5 minutos y menor a .',
      });
    }
    if (isEnable == undefined) {
      return res.status(400).json({
        message: 'Debe haber un valor (Si o No) en ¿Sucursal habilitada?.',
      });
    }
    if (!validate.isEnable(isEnable)) {
      return res.status(400).json({
        message:
          'El valor de en ¿Sucursal habilitada? es inválido. Debe ser booleano.',
      });
    }
    if (schedule && !validate.schedule(schedule)) {
      return res.status(400).json({
        message:
          'Formato de horario inválido. Verifique el formato de los días y horas deshabilitadas.',
      });
    }
  ...
  updateBranch: async (req, res) => {
    ...
    if (isEnable !== undefined && !validate.isEnable(isEnable)) {
      return res
        .status(400)
        .json({ message: 'Valor de isEnable inválido. Debe ser booleano.' });
    }
    if (schedule && !validate.schedule(schedule)) {
      return res.status(400).json({
        message:
  ...
  deleteBranch: async (req, res) => {
    ...
    try {
      ...
      if (users.length > 0) {
        return res.status(400).json({
          message:
            'No se puede eliminar la sucursal porque hay usuarios que la referencian.',
        });
      }
  ...
  getAssignedBranches: async (req, res) => {
    ...
    try {
      if (req.user.role !== 'oper') {
        return res
          .status(403)
          .json({ message: 'No autorizado. Acceso restringido a operadores.' });
      }
      ...
      if (assignedBranches.length === 0) {
        return res.status(404).json({
          message: 'No se encontraron sucursales asignadas al operador.',
        });
      }
      ...
      res
        .status(500)
        .json({ message: 'Error al obtener las sucursales asignadas.' });
    }
  },
  getAllBranches: async (req, res) => {
    try {
      ...
      if (!branches || branches.length === 0) {
        return res
          .status(404)
          .json({ message: 'No se encontraron sucursales.' });
      }
      res.json(branches);
    } catch (error) {
      console.error(error);
  ...
  getBranchById: async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: 'Error al obtener la información de la sucursal.' });
    }
  },
  getBranchSchedules: async (req, res) => {
    ...
    if (!queryDate) {
      return res
        .status(400)
        .json({ message: 'No hay una fecha válida pasada como params.' });
    }
    if (!validate.isValidDate(queryDate)) {
      return res
        .status(400)
        .json({ message: 'No pasaste una fecha válida en params.' });
    }
    try {
      const branch = await Branch.findByPk(branchId, {
      ...
      const allSchedules = reservationStepper.generateSchedules(
        branch.openingTime,
        branch.closingTime,
        branch.turnDuration
      );
      ...
      const availableSchedules = reservationStepper.filterAvailableSchedules(
        allSchedules,
        reservations,
        queryDate
      );
  ...
  getAvailableBranchSchedules: async (req, res) => {
    ...
    if (!queryDate) {
      return res
        .status(400)
        .json({ message: 'Fecha de consulta no proporcionada.' });
    }
    if (!validate.isValidDate(queryDate)) {
    ...
    try {
      ...
      if (!branch.isEnable) {
        return res
          .status(403)
          .json({ message: 'La sucursal no está habilitada.' });
      }
      const allSchedules = reservationStepper.generateSchedules(
        branch.openingTime,
        branch.closingTime,
      ...
      let availableSchedules = reservationStepper.filterSchedulesByDate(
        allSchedules,
        branch.schedule,
        branch.specificDates,
        queryDate
      );
      const reservations = await Reservation.findAll({
        where: {
      ...
      availableSchedules = reservationStepper.filterAvailableSchedules(
        availableSchedules,
        reservations,
        branch.capacity,
        queryDate
  ...
  getCriticalBranchSchedules: async (req, res) => {
    ...
    try {
      ...
      const criticalSchedules = reservationStepper.identifyCriticalSchedules(
        allSchedules,
        reservations,
        queryDate
      );
  ...
  updateSchedule: async (req, res) => {
    ...
    try {
      ...
      res.json({
        message: 'Horario de sucursal actualizado con éxito.',
        branch,
      });
    } catch (error) {
      console.error(error);
      res
        .status(500)
        .json({ message: 'Error al actualizar el horario de la sucursal.' });
    }
  },
  updateBranchEnableStatus: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        message: 'Estado de habilitación actualizado con éxito.',
        branch,
      });
    } catch (error) {
      console.error(error);
