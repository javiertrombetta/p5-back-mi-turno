...
const userController = {
  register: async (req, res) => {
    ...
    if (!validate.dni(dni)) {
      return res.status(400).json({
        message:
          "El DNI no cumple con las condiciones mínimas:\n" +
    ...
    if (!phoneNumber) {
      return res
        .status(400)
        .json({ message: "Número de teléfono no proporcionado." });
    }
    if (!validate.phone(phoneNumber)) {
      return res
        .status(400)
        .json({
          message: "El número de teléfono tiene un formato incorrecto.",
        });
    }
    if (!password) {
  ...
  getAllOpersByBusiness: async (req, res) => {
    try {
      ...
      if (opers.length === 0) {
        return res
          .status(404)
          .json({ message: "No se encontraron operadores para esta empresa." });
      }
  ...
  assignBusinessAndBranches: async (req, res) => {
    ...
    if (!branchIds || branchIds == undefined || branchIds == []) {
      return res
        .status(400)
        .json({ message: "No se enviaron las sucursales solicitadas." });
    }
  ...
  assignRoleToUser: async (req, res) => {
    ...
    if (!validate.dni(dni)) {
      return (
        res.status(400), json({ message: "El DNI proporcionado es inválido." })
      );
    }
  ...
  sendMessage: async (req, res) => {
    ...
    try {
      const userMailOptions = emailTemplates.userQueryConfirmation(
        user,
        message
      );
      await transporter.sendMail(userMailOptions);

      const adminMailOptions = emailTemplates.adminNotificationOfUserQuery(
        adminEmail,
        user,
        message
      );
